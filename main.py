...
def wait_for_idle(addr, idle_value, label):
    while True:
        try:
            r = bus.read_byte(addr)
            print(f"[{label}] Status von {hex(addr)}: {r}")
            if r == idle_value:
                print(f"[{label}] abgeschlossen (IDLE).")
                return True
        except:
            pass
        time.sleep(0.2)
        ...
        while not shutdown_event.is_set():
            ...
            if not dial_queue.empty() and dial_queue.get() == "start":
                ...
                for stueck in range(1, 11):
                    ...
                    if not wait_for_idle(TURNTABLE_ADDR, TURN_IDLE, "Drehteller ASM"):
                        break
                    ...
                    if not wait_for_idle(ROBO_ADDR, ROBO_IDLE, "Roboter MOVE_A"):
                        break
                    ...
                    if not wait_for_idle(TURNTABLE_ADDR, TURN_IDLE, "Drehteller SND"):
                        break
